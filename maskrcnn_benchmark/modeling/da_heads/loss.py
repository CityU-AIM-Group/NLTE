...
class CADALossComputation(object):
    """
    This class computes the DA loss.
    """
    ...
    def __call__(self, tn_grl_fea, da_ins_labels):

        da_tn_loss = F.binary_cross_entropy_with_logits(torch.squeeze(tn_grl_fea), da_ins_labels.type(torch.cuda.FloatTensor), reduction='none')
        return da_tn_loss

def make_ca_da_heads_loss_evaluator(cfg):
    loss_evaluator = CADALossComputation(cfg)
    return loss_evaluator